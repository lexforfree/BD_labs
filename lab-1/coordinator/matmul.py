...
Prompts for matrix size N, then runs A[NxN] @ B[NxN] both ways and compares.
...
async def main() -> None:
    ...
    print("-"*10)
    print("Matrix Multiplication: A[NxN] @ B[NxN]")
    ...
    if N <= NUMPY_MAX_SIZE:
        ...
        print(f"[numpy]\t\t{N}x{N} @ {N}x{N} -> {t_np:.4f}s")
    else:
        C_numpy = None
        print(f"[numpy]\tskipped — N={N} exceeds NUMPY_MAX_SIZE={NUMPY_MAX_SIZE}")
    ...
    print(f"[MapReduce]\t{N}x{N} @ {N}x{N}  ->  {t_mr:.4f}s")
